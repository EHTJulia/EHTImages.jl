function get_imextent(
    ...
    # check angular units
    dopixel = false
    if angunit isa Nothing
        aunit = get_unit(image.da.metadata["angunit"])
    ...
    if dopixel == false
        ...
        nx, ny = size(image.da.data)[1:2]
        dx = image.da.metadata["dx"]
        dy = image.da.metadata["dy"]
        ixref = image.da.metadata["ixref"]
        iyref = image.da.metadata["iyref"]
